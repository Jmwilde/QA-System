...
def lemmatize_list(l):
    lemmatized_list = pos([e for e in l if e.strip()])
...
def read_con_parses(parfile):
    ...
    lines = fh.readlines()
    fh.close()
    return [Tree.fromstring(line) for line in lines if line.strip()]
...
# arg type: (trees, list)
# return type: string
def where(tree, parse_trees, base_answer, filtered_question, question, difficulty):
    where = base_answer

    locative_verbs = ['to', 'onto', 'in', 'along', 'on', 'under', 'near', 'at', 'was in', 'into', 'upon', 'over']
            ...
            for subtree in tree.subtrees():
                if (word in lemmatized_tree):
                    if len((" ".join(subtree.leaves())).split(' ')) > 1:
                        phrase = (" ".join(subtree.leaves())).lower()
                        if (phrase.split(' '))[0] in locative_verbs:
                            loc_verb = phrase.split(' ')[0]
...
def what(question, question_dep_graph, story_dep_trees, story_parse_trees, filtered_question, difficulty):
    # Try to analyze the meaning of the question's action
    return_answer = smart_what(question, question_dep_graph, story_dep_trees, filtered_question)
...
def smart_what(question, question_dep_graph, story_dep_trees, filtered_question):
...
# HOW QUESTION -------------------------------------------------------------------
##################################################################################
def how(filtered_question, base_answer):
    last_word = filtered_question[len(filtered_question)-1]
    lemmatized_answer = lemmatize_list(base_answer.split(' '))
    i = 0
    for x in lemmatized_answer:
        i += 1
        if (x == last_word):
            return lemmatized_answer[i]

##################################################################################
# WHEN QUESTION ------------------------------------------------------------------
##################################################################################
def when(tree, base_answer):
    return base_answer
    ...
    for file in question_files:
        ...
        for dictionary in dictionary_list:
            ...
            if (q_type == "where"):
                ...
                for s,t in zip(sentences, parse_trees):
                    if (s == base_answer):
                        tree = t
                answer = where(tree, parse_trees, base_answer, filtered_question, question, difficulty)
            elif (q_type == "when"):
                print(file[1]+story_type+'.par')
                parse_trees = read_con_parses(file[1] + story_type + '.par')
                for s,t in zip(sentences, parse_trees):
                    if (s == base_answer):
                        tree = t
                answer = when(tree, base_answer)
            elif (q_type == "how"):
                answer = how(filtered_question, base_answer)
            elif (q_type == "who"):
                ...
                parse_trees = read_con_parses(file[1] + story_type + '.par')
                answer = what(question, qgraphs[qID], sgraphs, parse_trees, filtered_question, difficulty)
            else:
                answer = base_answer
            if answer is None:
                answer = ''
